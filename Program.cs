            ...
            do
            {
                ...
                switch (caso)
                {
                    case 1:
                        ...
                        var nuevoTitular = new Titular(nombre, idGenerado);
                    ...
                    case 2:
                        ...
                        var idBuscado = Console.ReadLine()?.Trim();

                        var titularSeleccionado = listaTitulares.FirstOrDefault(t => t.ID == idBuscado);

                        if (titularSeleccionado != null)
                        {
                            Console.Write("Ingrese el monto: ");
                            if (double.TryParse(Console.ReadLine(), out var monto))
                            {
                    ...
                    case 3:
                        Console.Write("Ingrese el ID del titular: ");
                        var idBuscadoRetiro = Console.ReadLine();

                        var titularSeleccionadoRetiro = listaTitulares.FirstOrDefault(t => t.ID == idBuscadoRetiro);

                        if (titularSeleccionadoRetiro != null)
                        {
                            Console.Write("Ingrese el monto a retirar: ");
                            if (double.TryParse(Console.ReadLine(), out var montoRetiro))
                            {
